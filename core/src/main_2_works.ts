...
const senderKey = senderAccount.key;
  ...
  const deployTxn = await Mina.transaction(
    ...
    async () => {
     // AccountUpdate.fundNewAccount(deployerKey.toPublicKey());
      ...
      //const storedHash=Field(42);
      const fieldArray=Field(42)
      zkApp.storedHash.set(Poseidon.hash([fieldArray]));
  ...
  const txn = await Mina.transaction(
    senderAccount,
    async () => {
      //console.log("Sender:",sender.zkAppAddress());
