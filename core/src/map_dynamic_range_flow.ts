...
function mapJsonToRiskData(jsonData: Contract[]): { inflow: number[][]; outflow: number[][] } {
  if (jsonData.length === 0) return { inflow: [], outflow: [] };
...
function loadAndProcessJson(filePath: string): { inflow: number[][]; outflow: number[][] } {
  const jsonData: Contract[] = JSON.parse(fs.readFileSync(filePath, "utf8"));
  return mapJsonToRiskData(jsonData);
}

export { mapJsonToRiskData, loadAndProcessJson };
